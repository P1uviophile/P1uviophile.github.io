const anzhiyu = {
  ...
  addEventListenerMusic: function () {
    const anMusicPage = document.getElementById("anMusic-page");
    if (!anMusicPage) return; // 非 music 页面直接退出
    ...
    metingAplayer.on("loadeddata", () => anzhiyu.changeMusicBg());
    ...
    aplayerIconMenu?.addEventListener("click", () => {
      ...
      anMusicPage.querySelector(".aplayer-list").style.opacity = "1";
    });
    ...
    document.addEventListener("keydown", function (event) {
      ...
      switch (event.code) {
        ...
        case "ArrowUp":
          event.preventDefault();
          if (musicVolume < 1) {
            musicVolume = Math.min(1, +(musicVolume + 0.1).toFixed(1));
        ...
        case "ArrowDown":
          event.preventDefault();
          if (musicVolume > 0) {
            musicVolume = Math.max(0, +(musicVolume - 0.1).toFixed(1));
            metingAplayer.volume(musicVolume, true);
          }
          break;
      }
